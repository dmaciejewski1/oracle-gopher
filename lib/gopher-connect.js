...
var gopherTime = require('./gopher-time.js');
var gopherResponse = require('./gopher-response.js');
var gopherTools = require('./gopher-tools')

function terminateConnection(connection, callback){
  connection.release(
    function(err) {
...
exports.sendTransaction = function (connectionObject,transactionObject,callback) {
  ...
  if (transactionObject.dbStatement){
     ...
     gopherTools.fetchBindVariableNames(
       ...
       function(err,res){
         ...
         //if bindVariableNames, than bindVariablesPresent = true, else false
  ...
  if (bindVariablesPresent === true && Object.keys(transactionObject.bindVariables).length <1){
    ...
    //build error object and append to transactionObject
 ...
 //-------------------------------------------
 //build the transaction configuration object (sent to the oracle driver)
    ...
    function(err, oraConnect) {
      ...
      oraConnect.execute(
        ...
        function (oraErr, dbResponse) {
          ...
          terminateConnection(oraConnect,function(terminationConnectionErr,terminationConnectionRes){
            //Handle connection termination errors...
            if(terminationConnectionErr){
              Object.assign(transactionObject,{"endTimeObject": gopherTime.getDateObject()});
              Object.assign(transactionObject,{"error":{"errType":'oracle',"errMsg": terminationConnectionErr}});
              return gopherResponse(connectionObject,transactionObject,'ERROR',
                function(gophErr,gophRes){
                  if(gophErr){callback(gophErr,'ERROR')}
...
exports.test = function (inputs, callback) {
  var connectionObject = {};
  var testSQLStr = "SELECT * FROM DUAL";

  for (var key in inputs) {
    if (inputs.hasOwnProperty(key)) {
        //set what keys/properties to look for, cache (from within the "inputs" object), and/or validate
        if (key === "user") {Object.assign(connectionObject,{"user":inputs[key]});}
        if (key === "password") {Object.assign(connectionObject,{"password":inputs[key]});}
        if (key === "connectString") {Object.assign(connectionObject,{"connectString":inputs[key]});}
    }
  }
  ...
  oracledb.getConnection(
    ...
    function(err, oraConnection) {
      if (err) {
        return callback(err.message,FAILMSG);
      }
      ...
      oraConnection.execute(
        ...
        function(err, result) {
          ...
          if (err) {
            oraConnection.release(function(err){
                   if (err) {return callback(err.message,FAILMSG);}
               });
            return callback(err.message,FAILMSG);
          }
          ...
          if (result.rows == 'X') {resMsg = PASSMSG;}
          else{resMsg = FAILMSG;}
        }
      );
    }
